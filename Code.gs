 ...
 *
 * ── 첨부파일(Drive) 업로드 오류가 날 때 ─────────────────────────
 * "DriveApp.getFoldersByName을(를) 호출할 수 있는 권한이 없습니다" 같은 오류가 뜨면,
 * 코드에 Drive 관련 기능을 새로 추가한 뒤 Drive 접근 권한을 아직 승인하지 않은 것입니다.
 * 1. Apps Script 편집기 상단의 함수 선택 드롭다운에서 "_getAttachFolder_" 선택
 * 2. ▶ 실행 버튼 클릭 → "권한 승인 필요" 팝업이 뜨면 [권한 검토] 클릭
 * 3. 본인 계정 선택 → "확인되지 않은 앱" 경고가 뜨면 [고급] 클릭 → [(프로젝트 이름)(으)로 이동] 클릭
 * 4. Google Drive 관련 권한에 체크된 상태로 [허용] 클릭
 * 5. 승인 후 상단 [배포] > [배포 관리] > 기존 배포 옆 연필(✏️) 아이콘 → 버전: "새 버전" → [배포]
 *    (URL은 그대로 유지되므로 앱 쪽 설정은 따로 바꿀 필요 없음)
 *
 * ── 참고 ──────────────────────────────────────────────────
