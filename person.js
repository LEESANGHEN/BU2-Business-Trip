...
// 표에는 한 줄로만 보여준다 (전체 출장일수 집계와는 무관 — 그건 항상 모든 출장 기준, _personTotalDaysUnion 참고).
// 대표는 "종료일이 가장 늦은(=가장 최신/진행 중인) 것"을 고른다 — 예전엔 "일수가 가장 긴 것"을 골랐는데,
// 그러면 이미 끝난 25일짜리 출장이 현재 진행 중인 19일짜리 출장보다 대표로 뽑혀서 상태가 "완료"로
// 잘못 표시되고, "진행중만 보기" 필터에서 실제로는 출장 중인 사람이 통째로 빠지는 문제가 있었다.
  ...
  Object.keys(bySite).forEach(function(siteId){
    ...
    clusters.forEach(function(c){
      ...
      c.items.forEach(function(t){
        if(t.end>rep.end||(t.end===rep.end&&t.tripTotal>rep.tripTotal)) rep=t;
      });
      result.push(rep);
    });
  });
